...
def test_connection() -> None:
    try:
        with db.connect() as conn:
            ...
            # Double-check the tables after commit
            tables = conn.execute(
                text(
                    "SELECT table_name FROM information_schema.tables WHERE table_schema = 'public'"
                )
            ).fetchall()
            logging.info(f"All tables in database after commit: {tables}")

            # Query second_table again
            query_result = conn.execute(text("SELECT * FROM second_table")).fetchall()
            logging.info(f"Query result from second_table after commit: {query_result}")
