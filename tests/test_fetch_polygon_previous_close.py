...
from models.polygon_models.polygon_previous_close import (
    PolygonDailyApiResponse,
    PreviousCloseResult,
)
from data_ingestion.fetch_polygon import fetch_polygon_previous_close
...
def test_fetch_polygon_previous_close(
    mock_constructor: Any, mock_service: Any, mock_client: Any, mock_get_data: Any
) -> None:
    mock_response = PolygonDailyApiResponse(
        adjusted=True,
        queryCount=1,
    ...
    mock_constructor.return_value.construct_previous_close_endpoint = MagicMock(
        return_value="mock_endpoint"
    )
    mock_service.return_value.fetch_data_with_endpoint.return_value = mock_response

    result = fetch_polygon_previous_close()
